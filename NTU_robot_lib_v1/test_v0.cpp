...
int main(int argc, char* argv[])
{
	...
	if (clientID != -1)
	{
		...
		float *atom_theta;
		float *atom_path_pos;
		float *atom_path_orient;
		atom_theta = new float[6];
		atom_path_pos = new float[3];
		atom_path_orient = new float[3];
		...
		while (simxGetConnectionId(clientID) != -1)
		{
			...
			simxGetObjectPosition(clientID, atom_pathHandle, atom_baseHandle, atom_path_pos, simx_opmode_streaming);
			simxGetObjectOrientation(clientID, atom_pathHandle, atom_baseHandle, atom_path_orient, simx_opmode_streaming);
			// Transfer the V-REP matrix form to roboop form
			for (int i = 1; i <= 3; i++)
				Tn(i, 4) = atom_path_pos[i-1];
			// get ik q;
			qn = robot_atom.inv_kin(Tn,2);
			robot_atom.set_q(qn);
			for (int i = 0; i < 6; i++)
				atom_set_qn[i] = (float)qn(i + 1);
			simxPauseCommunication(clientID, 1);
