...
class Operational_Space{
public:
	Operational_Space(const short dof=1);
	Operational_Space(const Robot_basic & robot, const DiagonalMatrix &Kp,
		const DiagonalMatrix &Kd, const DiagonalMatrix &Kf);  // setting the control gain
	ReturnMatrix torque_operational(Robot_basic &robot,const ColumnVector x,
		const ColumnVector xd, const ColumnVector xdd,
		const ColumnVector Fd, const ColumnVector Fext);
	short set_Kp(const DiagonalMatrix & Kp);  // set cartesian position gain
	short set_Kd(const DiagonalMatrix & Kd);  // set cartesian velocity gain
	short set_Kf(const DiagonalMatrix & Kf);  // set external force gain
...
private:
	...
	DiagonalMatrix Kp,  //!< Position error gain.
		Kd,  //!< Velocity error gain.
		Kf;  //!<external force control gain
