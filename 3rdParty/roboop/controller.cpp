...
Operational_Space::Operational_Space(const short dof_)
{
	dof = dof_;
	q = ColumnVector(dof); q = 0;
	qp = q;
...
Operational_Space::Operational_Space(const Robot_basic & robot, const DiagonalMatrix &Kp,
	const DiagonalMatrix &Kd, const DiagonalMatrix &Kf)
{
	dof = robot.get_dof();
	set_Kp(Kp);
	set_Kd(Kd);
...
short Operational_Space::set_Kp(const DiagonalMatrix & Kp_)
{
	if (Kp_.Nrows() != 6)
	{
		Kp = DiagonalMatrix(6); Kp = 0;
		cerr << "Operational_Space::set_kp: wrong size for input matrix Kp." << endl;
		return WRONG_SIZE;
	}
...
short Operational_Space::set_Kd(const DiagonalMatrix & Kd_)
{
	if (Kd_.Nrows() != 6)
	{
		Kd = DiagonalMatrix(6); Kd = 0;
		cerr << "Operational_Space::set_kd: wrong size for input matrix Kd." << endl;
		return WRONG_SIZE;
	}
...
short Operational_Space::set_Kf(const DiagonalMatrix & Kf_)
{
	if (Kf_.Nrows() != 6)
	{
		Kf = DiagonalMatrix(6); Kf = 0;
		cerr << "Operational_Space::set_kf: wrong size for input matrix Kf." << endl;
		return WRONG_SIZE;
	}
...
ReturnMatrix Operational_Space::torque_operational(Robot_basic &robot, const ColumnVector x,
	const ColumnVector xd, const ColumnVector xdd,const ColumnVector Fd, const ColumnVector Fext)
{
	ColumnVector e(6), ed(6), u_force(6), u_acc(6), p(3);  // position error, velocity error and control input
	ColumnVector xdn(6); // currently cartesian velocity
	Matrix Tn, Td;  //  Tn = Currently end-effector transformation, Td = Desired transformation matrix
	Matrix Jn, Jc;  // currently robot pose jacobian matrix Jn ; constraint Jacobain matrix Jc
	ColumnVector qn(dof), qdn(dof);
	const Real s0 = 10e-4;  // constraint jacobian value
	int singularbool = 0;  // Singular bool
	
	Matrix U, V;
	DiagonalMatrix S;
	qn = robot.get_available_q();
	qdn = robot.get_available_qp();
	Tn = robot.kine();
	for (int i = 1; i <= 3; i++)
		p(i) = x(i);
	Td = trans(p); Td = Td*rotx(x(4))*roty(x(5))*rotz(x(6));
	e = Kp*tr2delta(Tn,Td);
	Jn = robot.jacobian();
	ed = Kd*(xd - Jn*qdn);
	u_force = Fd + Kf*(Fd - Fext);  //  force control input
	u_acc = xdd + e + ed;   //  operaional space control input

	SVD(Jn, S, U, V);
	//  initial the value if without the singular direction
	Jc = Jn;
	xdn = xd;
	for (int i = 1; i <= dof; i++)
	{
		if (S(i) <= s0)  // if face the singular direction, something turn constrainted!!
		{
			...
			//  remove the singular direction
			for (int j = 1; j < i; j++)
				S_sing(j) = S(i);
			for (int m = 1; m <= U.Nrows(); m++){
				for (int n = 1; n < i; n++){
					U_sing(m, n) = U(m, n);
				}
			}
			//  Calculate the constrainted jacobian matrix
			Jc = S_sing*V.t();
			//  Redefine the control input into the null space direction
			u_acc.resize_keep(i - 1);
			u_force.resize_keep(i - 1);
			xdn.ReSize(i - 1);
			for (int j = 1; j < i; j++){
				//u_force(j) = Fd(j) + Kf(j)*(Fd(j) - Fext(j));
				xdn(i) = xd(i);
			}
			singularbool = 1;
			break;
		...
		Matrix M, Mx;
		M = robot.inertia(qn);   // Robot inertia
		Mx = Jc.i().t()*M*Jc.i();  // Cartesian inertia
		Matrix Jdot;   // Jacobian dot
		Jdot = robot.jacobian_dot();
		Matrix Cx, Gx;  // Cartesian centrifugal and gravity
		Cx = Jc.i().t()*robot.C(qdn);
		Gx = Jc.i().t()*robot.G();
		Matrix Jc_bar, Fc, N;   //  constraint Jacobain,  Force and nullspace
		IdentityMatrix idof(dof);
		Jc_bar = Mx*Jc*M.i();
		N = idof - Jc.t()*Jc_bar;
		ColumnVector Nro(dof), v1;
		Real v0;
		if (singularbool == 1){
			v1 = (Jn.determinant() - s0);
			v0 = v1.sum_absolute_value();
			Nro = N.t()*(-M*v0);
		}
		else{
			  // Nro = zeros(dof,1)
		}
		Fc = Mx*u_acc + Cx + Gx + u_force - Mx*xdn;
		tau = Jc.t()*Fc + Nro;
