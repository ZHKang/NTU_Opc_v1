...
//  derivative transformation matrix
ReturnMatrix tr2delta(const Matrix & Tn, const Matrix & Td)
{
	Matrix R0(3,3), R1(3,3), Rscew(3,3), delta(6,1);
	ColumnVector delta_p(3), delta_r(3);
	IdentityMatrix i3(3);
	R0 = t2r(Tn);R1 = t2r(Td);
	//  compute the delta position
	for (int i = 1; i <= 3; i++)
		delta_p(i) = Td(i, 4) - Tn(i, 4);  //  delta x,y,z
	//  compute the delta rotation
	Rscew = R1*R0.t() - i3;
	delta_r(1) = 0.5*(Rscew(3, 2) - Rscew(2, 3));
	delta_r(2) = 0.5*(Rscew(1, 3) - Rscew(3, 1));
	delta_r(3) = 0.5*(Rscew(2, 1) - Rscew(1, 2));
	...
	delta(5, 1) = delta_r(2);
	delta(6, 1) = delta_r(3);
	delta.Release(); return delta;
}

// translation and rotation seperation
ReturnMatrix t2r(const Matrix & T)
{
	Matrix R(3, 3);
	for (int i = 1; i <= 3; i++)
	{
