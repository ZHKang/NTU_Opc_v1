...
#define NITMAX 2000  //!< def maximum number of iterations in inv_kin 
#ifdef USING_FLOAT //from newmat's include.h
#  define ITOL   1e-2  //!< def tolerance for the end of iterations in inv_kin 
#else
#  define ITOL   1e-2  //!< def tolerance for the end of iterations in inv_kin 
#endif
...
*/
{
   ...
   else if (mj==1) {  // using partial derivative of T
   ...
   else   // Jacobian Damping
   {
	   Matrix Ipd, A, B(6, 1), Jd;
	   IdentityMatrix II(6);
	   float lumda = 0.001*0.001;
	   for (int j = 1; j <= NITMAX; j++) {
		   Ipd = (kine(endlink)).i()*Tobj;
		   B(1, 1) = Ipd(1, 4);
		   B(2, 1) = Ipd(2, 4);
